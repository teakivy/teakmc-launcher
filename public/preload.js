...
const launcherVersion = '0.0.1-beta';
...
contextBridge.exposeInMainWorld('api', {
	...
	instances: {
		...
		async playInstance(id) {
			ipcRenderer.send('launchingPercentz', { status: 'launching', percent: 0 });
				...
				.then(async (result) => {
					...
					launcher.on('debug', (e) => {
						...
						if (e.startsWith('[MCLC]: Using Java')) {
							ipcRenderer.send('launchingPercentz', { status: 'launching', percent: 20 });
						}
						if (e.startsWith('[MCLC]: Set native')) {
							ipcRenderer.send('launchingPercentz', { status: 'launching', percent: 30 });
						}
						if (e.startsWith('[MCLC]: Collected')) {
							ipcRenderer.send('launchingPercentz', { status: 'launching', percent: 35 });
						}
						if (e.startsWith('[MCLC]: Using ;')) {
							ipcRenderer.send('launchingPercentz', { status: 'launching', percent: 45 });
						}
						if (e.startsWith('[MCLC]: Attempting')) {
							ipcRenderer.send('launchingPercentz', { status: 'launching', percent: 50 });
						}
						if (e.startsWith('[MCLC]: Downloaded')) {
							ipcRenderer.send('launchingPercentz', { status: 'launching', percent: 70 });
						}
						if (e.startsWith('[MCLC]: Set launch')) {
							ipcRenderer.send('launchingPercentz', { status: 'launching', percent: 75 });
						}
						if (e.startsWith('[MCLC]: Launching')) {
							ipcRenderer.send('launchingPercentz', { status: 'launching', percent: 100 });
							setTimeout(() => {
								ipcRenderer.send('launchingPercentz', { status: 'launching', percent: 100 });
								setTimeout(() => {
	...
	accounts: {
		...
		async isDemoUser(uuid) {
			return msmc.isDemoUser(JSON.parse(await keytar.getPassword('accounts', uuid)));
		},
		...
		async findAccounts() {
			...
			for (const uuid in config.accounts) {
				accounts.push({
					uuid: uuid,
					username: JSON.parse(await keytar.getPassword('accounts', uuid)).profile.name,
		...
		login() {
			msmc
				...
				.then((result) => {
					...
					let acId = result.profile.id.includes('-') ? result.profile.id : dashUUID(result.profile.id);
