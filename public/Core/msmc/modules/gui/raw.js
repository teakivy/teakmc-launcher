...
function browserLoop(token, port, updates, browser) {
	return new Promise((resolve) => {
		...
		const f3 = setInterval(() => {
			BE.getFetch()('http://127.0.0.1:' + port + '/json/list')
				.then((r) => r.json())
				.then((out) => {
					for (var i = 0; i < out.length; i++) {
						...
						if (loc && loc.startsWith(token.redirect)) {
							...
							if (urlParams) resolve(MSMC.authenticate(urlParams, token, updates));
							else resolve({ type: 'Cancelled', translationString: 'Cancelled.Back' });
							call();
						}
					}
				})
